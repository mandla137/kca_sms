...
<?php if($class_id > 0 && $student_id > 0 && $exam_id > 0):?>	
	...
	<div class="col-sm-12">
        ...
        <?php $select_subject_with_class_id  =   $this->crud_model->get_subjects_by_class($class_id);
		  ...
		  <?php  $select_subject_with_mark_id  =   $this->crud_model->get_subjects_by_class($mark_id);
            foreach ($select_subject_with_mark_id as $key => $class_subject_exam_student):
			$query = $this->db->get_where('mark', $total_score);
              
                foreach ($total_score as $key => $general_select):
			
			$total_score = 1st_score + 2nd_score + 3rd_score + exam_score;	
                $grade = "A";
           ...
           ?>
			...
			<?php echo form_open(base_url() . 'teacher/marks/'. $exam_id . '/' . $class_id);?>
						<tr>
											...
											<td>
												<input type="char" class="exam_score form-control" value="<?php echo $general_select['grade'];?>" name="grade<?php echo $class_subject_exam_student['subject_id'];?>" onchange="grade_change()">
											</td>
